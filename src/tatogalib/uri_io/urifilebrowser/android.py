...
class UriFileBrowserImpl:
    ...
    async def open_file_dialog(self, title, initial_uri, file_types, multiselect):
        ...
        if initial_uri is not None and len(initial_uri) > 0:
            intent.putExtra(
                "android.provider.extra.INITIAL_URI", Uri.parse(initial_uri)
            )
            ...
            if result["resultData"] is not None:
                ...
                if data is None:
                    if multiselect:
                        ...
                        if (
                            clip_data is not None
                        ):  # just to be sure there will never be a null reference exception...
                            for i in range(0, clip_data.getItemCount()):
                                selected_uri.append(
                                    clip_data.getItemAt(i).getUri().toString()
                                )
                else:
