...
class MainGui(TaGui):
    ...
    def clip_copy(self, widget):
        try:
            cb = Clipboard(self.fnPrintln)
            cb.set_text(self.message_area.value)
            self.fnPrintln("Copied to clipboard")
        except BaseException as ex:
           G.write_debug_message(str(ex))
           self.fnPrintln("\n"+str(ex))
    # clip_copy

    def clip_paste(self, widget):
        try:
            cb = Clipboard(self.fnPrintln)
            self.message_area.value += cb.get_text()
        except BaseException as ex:
           G.write_debug_message(str(ex))
           self.fnPrintln("\n"+str(ex))
    # clip_paste
